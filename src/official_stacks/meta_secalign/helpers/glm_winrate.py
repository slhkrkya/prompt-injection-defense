...
def get_length_controlled_winrate(
    ...
    #print(f"Using GLM: {glm_name} with formula: {glm_info['formula']}"); exit()
...
def _get_featurized_data(
    ...
    if 'instruction_difficulty' in formula: df["instruction_difficulty"] = df["index"].transform(lambda g: instruction_difficulty[g])
    ...
    if 'instruction_difficulty' in formula: df_test = df[["instruction_difficulty", "not_gamed_baseline"]].copy()
    else: df_test = df[["not_gamed_baseline"]].copy()
